...
MagnoliaClient.prototype.create = function(node) {
  ...
  return fetch(endpoint, {
    ...
    .then(Errors.checkResponse)
    .then(() => Promise.all(node.nodes.map(n => this.create(n))))
    .then(() => this.getNode(node.path));
};

MagnoliaClient.prototype.createPage = function(page) {
  return this.create(page)
    .catch(ResponseError, e => {
      if (e.status !== 404) throw e;

      // 404 means this path's parent hasn't been created yet, so create it
      let parentPath = page.path.substr(0, page.path.lastIndexOf('/'));
      return this.createPage(Node.newPage(parentPath, page.getProperty('mgnl:template'))).then(() => this.createPage(page));
    });
};
...
MagnoliaClient.prototype.save = function(node) {
  ...
  return Promise.all([...node._removedProperties].map(name => {
    ...
    .then(() => Promise.all(node.nodes.map(n => this.save(n))));
};
